...
function Cardapio() {
  ...
  return <><PageIntro eyebrow="Cardápio" title="Sabores para todos os momentos." text="Conheça as categorias da casa. Os itens, ingredientes e preços serão incluídos após o recebimento do cardápio oficial." />
    <section className="px-5 py-20 lg:px-8 lg:py-28"><div className="mx-auto max-w-7xl">
      <div className="mb-10 rounded border border-brand-gold bg-secondary p-5 text-sm leading-6"><strong>Cardápio demonstrativo.</strong> Esta apresentação não contém nomes de pratos, ingredientes ou preços. O conteúdo oficial poderá substituir estes espaços.</div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{categories.map(([Icon, title], index) => <article key={title} className={`min-h-56 rounded border border-border p-7 ${index === 0 ? "bg-primary text-primary-foreground" : "bg-card"}`}><Icon className="size-7" /><p className="mt-12 text-xs font-bold uppercase tracking-[0.2em] opacity-70">Categoria {String(index + 1).padStart(2, "0")}</p><h2 className="mt-2 font-display text-4xl">{title}</h2><p className="mt-3 text-sm opacity-75">Espaço preparado para receber os itens oficiais desta categoria.</p></article>)}</div>
